...
- (id) initWithFrame:(CGRect)frame friends:(NSMutableArray*)friends circles:(NSMutableArray*)circles
{
    ...
    if (self) {
        ...
        for (int i=0; i < circleList.count; i++ ) {
            UserCircle *aCircle = [circleList objectAtIndex:i];
            NSLog(@"SelectFriends:filtered circle:%@", aCircle.circleID);
        ...
        // Change 'Search' button on keyboard to 'Done'. Also, make 'Done' button active when no search input
        ...
        //
        photoSel = [[UIView alloc] initWithFrame:CGRectMake(0, line2Frame.origin.y+7, self.frame.size.width, 250)];
...
- (void)drawRect:(CGRect)rect
{
    ...
    // Circles
    // Remove all subviews
    for (UIView *aView in [circleScrollView subviews]) {
        [aView removeFromSuperview];
    }
    CGRect circleScrollFrame = CGRectMake(0, 0, self.frame.size.width, 210);
    for (int i=0;i<circleList.count;i++) {
        ...
        CustomCheckbox *circle = [[CustomCheckbox alloc] initWithFrame:CGRectMake(0, i*(5+45), circleScrollFrame.size.width, 45) 
                                                            boxLocType:LabelPositionLeft 
                                                              numBoxes:1 
                                                               default:[NSArray arrayWithObject:[NSNumber numberWithInt:def]] 
...
{   
    NSLog(@"SelectFriends textDidChange: text=[%@]", searchText);
