...
public class RemoteImageCache {
	...
	private Set<String> bad = Collections
			.synchronizedSet(new HashSet<String>());
	private Set<String> down = Collections
			.synchronizedSet(new HashSet<String>());
	...
	private class DownloaderThread extends Thread {
		...
		public void run() {
			...
			while (active.get()) {
				...
				final File imageFile = new File(storageDir, imageInfo.id
						+ ".img");

				Bitmap bitmap = FBUtility.getBitmap(imageInfo.imageUrl);
				if (bitmap != null) {
					throttle.release();
					...
					try {
						...
						bitmap.compress(imageInfo.format, imageInfo.quality,
								baos);
					...
					handler.post(new Runnable() {
						...
						public void run() {
							...
							// run the ImageCacheListner callback
							// for success
							if (imageInfo.listener != null) {
								imageInfo.listener.onSuccess(
										imageInfo, image);
							}
						}
					});
				...
				{
					// image download failed, release semaphore, let
					// the next one go
					throttle.release();

					// remove from the downloading state, add to bad
					// images so we
					// won't try to download again
					bad.add(imageInfo.id);
					down.remove(imageInfo.id);

					// create a handler to ensure the callback
					// listener runs in the
					// main UI thread, pass in the error and the
					// original image info
					handler.post(new Runnable() {
						...
						public void run() {
							...
							// for failure
							if (imageInfo.listener != null) {
								imageInfo.listener.onFailure(null,
										imageInfo);
							}
						}
					});
				...
				/*httpClient.get(imageInfo.imageUrl, null,
						new BinaryHttpResponseHandler() {
							...
							public void onSuccess(byte[] bytes) {
								...
								// image downloaded, release semaphore, let the
								// next one go
								throttle.release();
								...
								try {
									...
									// if we are sampling, then sample the image
									// and turn it into a
									// Bitmap, if not just turn it into a Bitmap
									if (imageInfo.sample) {

										bitmap = BitmapUtils
												.decodeAndScaleImage(bytes,
														imageInfo.width,
														imageInfo.height);
										ByteArrayOutputStream baos = new ByteArrayOutputStream();
										bitmap.compress(imageInfo.format,
												imageInfo.quality, baos);
										imageBytes = baos.toByteArray();
									} else {
								...
								// create a handler to ensure the callback
								// listener runs in the
								// main UI thread, pass in the Bitmap and the
								// original image info
								final Bitmap image = bitmap;
								handler.post(new Runnable() {
							...
							public void onFailure(final Throwable error,
								...
								handler.post(new Runnable() {
									...
									public void run() {
										...
										// for failure
										if (imageInfo.listener != null) {
											imageInfo.listener.onFailure(error,
													imageInfo);
										}
									}
								});
	 ...
	 * @param maxParallelDown
	 *            The maximum number of downloads that can happen concurrently.
	 * @param cacheDir
	 *            The local directory to store cached images. This is just a
	 *            directory name. The directory will always be inside the
	 *            applications data/files directory.
	 * @param cacheSize
